...
/** Solve dense sparse LAP. Takes and returns fortran-style arrays. Only works on squares
 */
void lapjv_fort(
    cost_t *sumSol, int* jSol,
    cost_t *cost, int *n_ref) {
    /// n_ref: size of matrix
    /// cost: column-major 2d array
    /// x, y: x and y indixes of result

    uint_t n = uint_t(*n_ref);

    cost_t *cost_copy = new cost_t[n*n];    
    std::copy(cost, cost+(n*n), cost_copy);

    cost_t **cost2d = new cost_t*[n];    
    for (uint_t i = 0; i < n; i++) {
        cost2d[i] = &(cost_copy[i*n]);
    }

    int* y = new int[n];
    *sumSol = 0;
    
    lapjv_internal(n, cost2d, jSol, y); 


    for (uint_t i = 0; i < n; i++) { // 1-indexing and total cost
        *sumSol += cost[i*n + jSol[i]];
        y[i]++;
        jSol[i]++;
    }
