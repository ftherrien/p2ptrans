module transform
  ...
  double precision, parameter ::  bond = 2.5d0
...
contains
  ...
  recursive function det(a,n) result(accumulation)
    ...
    distance = sum(bond**12/d**6 - 2*bond**6/d**3)
...
!       sum(-(sum((Apos - free_trans(Bpos,M,vec))**2,1)+9.0d0)**(-3))
       ...
       P = bond**6/(sum(E**2,1))**7 - 1/(sum(E**2,1))**4
       E = -E*spread(P,1,3)
...
!       sum(-(sum((Apos - free_trans(Bpos,M,vec))**2,1)+9.0d0)**(-3))
    ...
    subroutine analytical_gd_free(tmat, vec, Apos, Bpos, sq, n_iter, rate1, rate2, max_vol)
    ...
    do j=1, n_iter
       ...
       do while (vol == 0)
          ...
          do vl=1, 10 ! TMP vec
             ...
             vec_local(3) = bond ! As an estimate
             ...
             do k=1, n_conv
                ...
                call analytical_gd_free(tmat_local, vec_local, Apos_mapped, Bpos_opt, &
                     .false.,n_ana*100, rate1, rate2, max_vol)
             ...
             write(*,*) "Opt dist", thread, dist_cur, vec_local(3), det(tmat_local,3)
             ...
             if (abs(det(tmat_local,3) - 1.0d0) < max_vol) then
                if (vl == 1 .or. dist_cur < stats(j,4)) then !TMP vec
                   ...
                   stats(j,2:3) = vec_local(1:2)
                   stats(j,4) = dist_cur
                   stats(j,5) = vec_local(3)
                   vol = det(tmat_local,3)
                   tmats(j,:,:) = tmat_local
  ...
  subroutine fastoptimization(Apos_out, Bpos_out, Bpos_out_stretch, &
    ...
    ! tmat = reshape((/-1.8730712394830298E-003, 1.0055339081726102, 0.0000000000000000, &
    !      -0.95119746032042263, -9.1898877204804302E-002, 0.0000000000000000, &
    !      0.0000000000000000, 0.0000000000000000, 1.0000000000000000/),(/3,3/))     
    ! vec = (/1.0904030230458825, -3.3059878638539759, 2.0713929040282446/) 
    ...
    if (.false.) then
       ...
       do i=1,n_iter
          ...
          vecs(i,1:2) = stats(idx(i),2:3)
          vecs(i,3) = stats(idx(i),5)
          stats(idx(i),5) = det(tmats(idx(i),:,:),3)
